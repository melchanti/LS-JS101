function searchWord (word, originalText) {
  if (arguments.length < 2) return 'one or more arguments are missing';

  let re = new RegExp (`\\b${word}\\b`, 'gi');
  
  let numMatches = originalText.match(re) ? originalText.match(re).length: 0;
  let returnedString = originalText.replace(re, `**${word.toUpperCase()}**`);

  console.log (returnedString);
  return numMatches;
}

const text = 'Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium ' + 
